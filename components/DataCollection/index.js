...
const DataCollection = Form.create()( ( props ) => {
	...
	const [ records, setRecords ] = useState( map( data, ( record, id ) => ( { id, key: id, ...record } ) ) );
	...
	const handleOk = () => {
		...
		if ( ! isEmpty( edit ) ) {
			const id = edit.id;
			save = [ ...records ];
			save.splice( id, 1, { id, key: id, ...values } );
		} else {
			const id = records.length;
			save = [ ...records, { id, key: id, ...values } ];
		}
